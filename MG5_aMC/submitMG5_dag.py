#!/usr/bin/env python
"""
Submit lots of MG5 samples to condor as a DAG to allow easy monitoring and retry.
"""
...
def submit(in_args=sys.argv[1:]):
    ...
    with open(dag_name, 'w') as dag_file:
        ...
        dag_file.write('# %d jobs each generating %d events\n' % (args.nJobs, args.nEvents))
        dag_file.write('\n')
    ...
    if not args.dry:
        ...
        print 'DAGstatus.py %s' % status_name
